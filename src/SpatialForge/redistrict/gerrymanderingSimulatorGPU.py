...
class GerrymanderSimulator:
    ...
    def score_map(self):
        ...
        compactness_scores = self.district_stats['perimeter'] / np.sqrt(self.district_stats['area'])
    ...
    def will_break_district(self, pixel_i, pixel_j, district_id):
        ...
        return self._static_will_break_district(self.district_map, pixel_i, pixel_j, district_id, 
    ...
    def _process_batch_multi(self, batch_size=100, pixels_per_move=10):
        """
        Process a batch of multi-pixel moves in parallel with multi-GPU support.
        
        This method optimizes the district boundaries by changing multiple boundary
        pixels simultaneously, which is especially effective in early phases.
        
        Parameters:
        - batch_size: Number of multi-pixel moves to make
        ...
        if self.use_gpu and self.num_gpus > 1:
            # For multi-GPU system, distribute work across GPUs
            if self.using_dask:
                # Use Dask for distributed processing
                futures = []
                ...
                for gpu_id, (start_row, end_row) in enumerate(self.partition_boundaries):
                    # Submit task to GPU
                    futures.append(self.client.submit(
                        self._process_batch_multi_partition,
                        gpu_id, batch_size // self.num_gpus, pixels_per_move, 
                        self.phase, self.temperature, start_row, end_row,
                        resources={'GPU': 1}
                    ))
                ...
                # Recalculate all stats after updates
                self.calculate_all_district_stats()
                return accepted_count
                
            else:
                # Manual multi-GPU approach
                results = []
                for gpu_id, (start_row, end_row) in enumerate(self.partition_boundaries):
                    with cp.cuda.Device(gpu_id):
                        result = self._process_batch_multi_partition(
                            gpu_id, batch_size // self.num_gpus, pixels_per_move,
                            self.phase, self.temperature, start_row, end_row)
                        results.append(result)
                ...
                # Update with best partition
                if best_idx >= 0:
                    start_row, end_row = self.partition_boundaries[best_idx]
                    with cp.cuda.Device(0):  # Use device 0 for updates
                        self.district_map[start_row:end_row] = cp.asarray(results[best_idx][0])
                
                # Recalculate stats
                self.calculate_all_district_stats()
                return accepted_count
        
        # Single GPU or CPU implementation
        ...
        for _ in range(batch_size):
            ...
            for i, j in selected_pixels:
                if self.use_gpu and not self.using_dask:
                    old_district = int(self.district_map[i, j].item())
                else:
                    old_district = self.district_map[i, j]
                
                # Find neighboring districts
                neighboring_districts = set()
                for ni, nj in self.neighbor_map.get((i, j), []):
                    if self.use_gpu and not self.using_dask:
                        neighboring_districts.add(int(self.district_map[ni, nj].item()))
                    else:
                        neighboring_districts.add(self.district_map[ni, nj])
                
                neighboring_districts.discard(old_district)
                
                if neighboring_districts and not self.will_break_district(i, j, old_district):
                    new_district = random.choice(list(neighboring_districts))
                    moves.append((i, j, old_district, new_district))
            ...
            # Make a backup of the current state
            if self.use_gpu and not self.using_dask:
                original_map = cp.copy(self.district_map)
                original_stats = {k: cp.copy(v) for k, v in self.district_stats_gpu.items()}
            else:
                original_map = self.district_map.copy()
                original_stats = {k: v.copy() for k, v in self.district_stats.items()}
            
            # Apply all moves
            for i, j, old_district, new_district in moves:
                self.district_map[i, j] = new_district
                self.update_district_stats(i, j, old_district, new_district)
            ...
            if new_score <= current_score:
                ...
                if random.random() < accept_probability:
                    # Accept the changes despite being worse
                    accepted_count += 1
                else:
                    # Revert all changes
                    if self.use_gpu and not self.using_dask:
                        self.district_map = original_map
                        for k in self.district_stats_gpu:
                            self.district_stats_gpu[k] = original_stats[k]
                        # Copy back to CPU stats
                        for k in self.district_stats:
                            self.district_stats[k] = cp.asnumpy(self.district_stats_gpu[k])
                    else:
                        self.district_map = original_map
    ...
    def get_boundary_pixels(self):
        ...
        return boundary_pixels
    def _process_batch_multi_partition(self, gpu_id, batch_size, pixels_per_move, phase, temperature, start_row, end_row):
        """
        Process a batch of multi-pixel moves on a specific GPU partition.
        ...
        - start_row, end_row: Row boundaries for this partition
        ...
        with cp.cuda.Device(gpu_id):
            ...
            for _ in range(batch_size):
                ...
                if len(boundary_pixels) <= pixels_per_move:
                    ...
                    indices = cp.random.choice(len(boundary_pixels), pixels_per_move, replace=False).get()
                    selected_pixels = [boundary_pixels[i] for i in indices]
            ...
            # Return the best map found and its score
            return best_map.get(), accepted_count, float(best_score)